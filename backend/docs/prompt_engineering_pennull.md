# penNULL Prompt Engineering Guide

A comprehensive framework for designing high-performance prompts within the penNULL penetration testing system. This guide provides specialized principles for creating prompts that leverage the multi-agent architecture, memory systems, security tools, and specific operational context of penNULL.
...
## Core Principles for penNULL Prompts
...
- Begin with a clear definition of the agent's specific **role** (e.g., Orchestrator, Pentester, Searcher), its primary **objective** within the penNULL workflow, and any overarching **security focus**.
...
- Ensure instructions are **specific**, **unambiguous**, use **active voice**, and are directly relevant to the agent's function within penNULL.
...
[Role definition, primary objective, and security focus relevant to penNULL]

## CORE CAPABILITIES / KNOWLEDGE BASE
[Agent-specific skills, knowledge areas relevant to penNULL tasks]
...
- Tailor instructions, tone, knowledge references, and complexity directly to the agent's specialized role within the penNULL system (Orchestrator, Pentester, Searcher, Developer, Adviser, Memorist, Installer). Explicitly reference `ai-concepts.mdc` for role definitions.
...
- Provide explicit, actionable instructions on *when* and *how* to interact with penNULL's vector memory system. Reference `ai-concepts.mdc` (Memory section).
...
- Include a complete, accurate roster of **all available specialist agents** within penNULL (searcher, pentester, developer, adviser, memorist, installer).
...
- **Emphasize Key Points:**
    ...
    - **Reinforce:** The penNULL system operates **exclusively via structured tool calls.** Any attempt to simulate actions or results in plain text will fail.
...
- Specify **resource limitations** (e.g., default command timeouts) and **operational restrictions** derived from penNULL's secure execution model (No GUI, No host access, No UDP scanning, No arbitrary software installation). Reference `security-tools.mdc`.
...
- Understand that newer LLMs (like those used in penNULL) follow instructions **more literally and precisely** than previous generations. Make instructions explicit and unambiguous, avoiding indirect or implied guidance.
...
- Design prompts with the understanding that penNULL agents will **follow the letter of instructions** rather than attempting to infer unstated intent.
...
- Ensure prompts utilize essential context variables provided by the penNULL backend:

...
The Google AI provider enables penNULL to use Google's Gemini language models through the Generative AI API. This provider supports advanced features like function calling, streaming responses, and competitive pricing.
...
docker run --rm \
  ...
  nullailab/pennull /opt/pennull/bin/ctester -type gemini
```
...
docker run --rm \
  -v $(pwd)/.env:/opt/pennull/.env \
  nullailab/pennull /opt/pennull/bin/ctester \
  -config /opt/pennull/conf/gemini.provider.yml

# Using custom configuration
docker run --rm \
  -v $(pwd)/.env:/opt/pennull/.env \
  -v $(pwd)/gemini-custom.yml:/opt/pennull/gemini-custom.yml \
  nullailab/pennull /opt/pennull/bin/ctester \
  -type gemini \
  -config /opt/pennull/gemini-custom.yml
```
...
### Cost Management
- Monitor token usage through penNULL's cost tracking
...
- [penNULL Documentation](https://docs.pennull.com)
- [Issue Tracker](https://github.com/nullailab/pennull/issues)

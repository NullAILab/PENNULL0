...
The Ollama provider enables penNULL to use local language models through the [Ollama](https://ollama.ai/) server.
...
Control how penNULL interacts with your Ollama server:
...
**Performance Note:** Enabling `OLLAMA_SERVER_LOAD_MODELS_ENABLED` adds startup latency as penNULL queries the Ollama API. Disable if you only need specific models from config file.
...
The provider supports all penNULL agent types with optimized configurations:
...
# Start penNULL with Ollama provider
./pennull
```

# Langfuse Integration for penNULL

This document provides a comprehensive guide to the Langfuse integration in penNULL, covering architecture, setup, usage patterns, and best practices for developers.

## Table of Contents

- [Langfuse Integration for penNULL](#langfuse-integration-for-pennull)
...
Langfuse is an open-source observability platform specifically designed for LLM-powered applications. The penNULL integration with Langfuse provides:
...
The Langfuse integration in penNULL is built around a layered architecture that provides both high-level abstractions for simple use cases and fine-grained control for complex scenarios.

```mermaid
flowchart TD
    Application[penNULL Application] --> Observer[Observer]
...
```mermaid
sequenceDiagram
    participant App as penNULL Application
...
| `LANGFUSE_INIT_USER_EMAIL` | Admin user email | admin@pennull.com |
...
The main integration point in penNULL is the Flow Controller, which handles the lifecycle of AI agent flows:

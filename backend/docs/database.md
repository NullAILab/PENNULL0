...
The `database` package is a core component of penNULL that provides a robust, type-safe interface for interacting with PostgreSQL database operations. Built on top of [sqlc](https://sqlc.dev/), this package automatically generates Go code from SQL queries, ensuring compile-time safety and eliminating the need for manual ORM mapping.

penNULL uses PostgreSQL with the [pgvector](https://github.com/pgvector/pgvector) extension to support vector embeddings for AI-powered semantic search and memory storage capabilities.
...
The database follows penNULL's hierarchical data model for penetration testing workflows:
...
```bash
docker run --rm -v "$(pwd):/src" --network pennull-network \
  -e DATABASE_URL='postgres://postgres:postgres@pgvector:5432/pennulldb?sslmode=disable' \
  -w /src sqlc/sqlc:1.27.0 generate -f sqlc/sqlc.yml
```

This command:
1. Mounts the current directory into the container
2. Connects to the penNULL database network
...
The database package integrates with penNULL's GraphQL API through the converter package:
...
**Log Output Example**:
```
INFO[0000] SELECT * FROM flows WHERE user_id = '1' AND deleted_at IS NULL  component=pennull-gorm duration=2.5ms rows_returned=3
```
...
The database package provides a robust, secure, and performant foundation for penNULL's data layer. By leveraging sqlc for code generation, implementing consistent security patterns, and maintaining comprehensive audit trails, it ensures reliable operation of the autonomous penetration testing system.
...
For developers working with this package, follow the established patterns for security, performance, and maintainability to ensure smooth integration with the broader penNULL ecosystem.

This documentation provides a comprehensive overview of the database package's architecture, functionality, and integration within the penNULL system.

...
- [Integration with penNULL](#integration-with-pennull)
...
The Docker client package (`backend/pkg/docker`) provides a secure and isolated containerized environment for penNULL's AI agents to execute penetration testing operations. This package serves as a wrapper around the official Docker SDK, offering specialized functionality for managing containers that AI agents use to perform security testing tasks.
...
### Role in penNULL Ecosystem

The Docker client is a critical component that enables penNULL's core promise of secure, isolated penetration testing. It provides the foundation for:
...
penNULL uses a deterministic port allocation algorithm to ensure each flow gets unique, predictable ports:
...
| `DOCKER_INSIDE` | `false` | Whether penNULL communicates with host Docker daemon from containers |
| `DOCKER_NET_ADMIN` | `false` | Whether penNULL grants the primary container NET_ADMIN capability for advanced networking. |
...
| `DOCKER_DEFAULT_IMAGE_FOR_PENTEST` | `nullailab/kali-linux` | Default Docker image for penetration testing tasks |
...
type Config struct {
    ...
    DockerDefaultImageForPentest string `env:"DOCKER_DEFAULT_IMAGE_FOR_PENTEST" envDefault:"nullailab/kali-linux"`
...
The `DOCKER_NET_ADMIN` option controls whether penNULL containers are granted the `NET_ADMIN` Linux capability, which provides advanced networking permissions essential for many penetration testing operations.
...
penNULL supports running inside Docker containers while still managing other containers. This is controlled by the `DOCKER_INSIDE` setting:

- **`DOCKER_INSIDE=false`**: penNULL runs on host, manages containers directly
- **`DOCKER_INSIDE=true`**: penNULL runs in container, mounts Docker socket to manage sibling containers

### Network Configuration

penNULL supports two network modes for container isolation:

#### Bridge Network Mode (Default)

When `DOCKER_NETWORK` is set to a custom network name (e.g., `pennull-network`), containers are connected to an isolated bridge network:
...
The main interface defines all Docker operations available to penNULL components:
...
penNULL tracks container states in the database:
...
func PrimaryTerminalName(flowID int64) string {
    return fmt.Sprintf("pennull-terminal-%d", flowID)
}
```

This creates names like `pennull-terminal-123` for flow ID 123, making it easy to:
...
penNULL implements a multi-layered security approach for container isolation:
...
## Integration with penNULL

### Tool Integration

The Docker client integrates with penNULL's tool system to provide terminal access:
...
- Integrate with penNULL's database for state management

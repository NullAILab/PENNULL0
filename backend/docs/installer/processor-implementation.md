...
- **State machine logic**: `ApplyChanges` implements three-phase stack management (Observability → Langfuse → penNULL) with integrity validation; the wizard performs a pre-phase interactive integrity check with Y/N decision for force mode
...
- **Three-phase execution**: Observability → Langfuse → penNULL with state validation after each phase

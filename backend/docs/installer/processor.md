...
graph TB
    ...
    subgraph "External Systems"
        ...
        UpdateServer[Update Server<br/>pennull.com]
...
- **ProductStack**: Logical grouping of services that can be managed as a unit (pennull, langfuse, observability, worker, installer, all)
...
- **Worker Images**: Container images for AI agent tasks (default: debian:latest, pentest: nullailab/kali-linux)
...
const (
    StackpenNULL        ProductStack = "pennull"        // Main stack (docker-compose.yml)
...
- **Start(stack)**:
  - pennull/langfuse/observability: `docker compose ... up -d` (honors embedded mode for non-destructive ops)
  - all: sequential start in order observability → langfuse → pennull
  - worker/installer: not applicable

- **Stop(stack)**:
  - pennull/langfuse/observability: `docker compose ... stop`
  - all: sequential stop in reverse order (pennull → langfuse → observability)
  - worker/installer: not applicable
...
- **Download(stack)**:
  - pennull/langfuse/observability: `docker compose pull`
...
- **Install(stack)**:
  - pennull: extract compose file and example provider config
...
- **Update(stack)**:
  - pennull/langfuse/observability: download → `docker compose up -d`
...
### Removal Operations
- **Remove(stack)**:
  - pennull/langfuse/observability: `docker compose down` (keep volumes/images)
  - worker: remove images via Docker API and related containers
  - installer: remove flow stubbed
  - all: remove all stacks

- **Purge(stack)**:
  - pennull/langfuse/observability: `down --rmi all -v` for strict purge; standard purge `down -v` is also available
...
- **ApplyChanges()**:
  ...
  - phase 3: pennull (always embedded; ensure/verify → update)
...
- **compose.go**:
  - `installpenNULL()`, `installLangfuse()`, `installObservability()` - extract compose files with environment patching
...
- **docker.go**:
  - `pullWorkerImage()` - download pentest image (DOCKER_DEFAULT_IMAGE_FOR_PENTEST: nullailab/kali-linux)
...
- **update.go**:
  - `checkUpdates()` - communicate with pennull.com update server
...
- **Two-Track Command Execution**:
  - Worker stack: Docker API SDK (uses DOCKER_HOST, PENNULL_DOCKER_CERT_PATH, DOCKER_TLS_VERIFY from config)
...
- **Progress Tracking**: Worker downloads (nullailab/kali-linux 6GB+ → 13GB disk) with real-time progress via terminal
- **Docker Configuration**: Support NET_ADMIN capability for network scanning, Docker socket access for container management
- **Dependency Ordering**: penNULL must start before Langfuse/Observability for network creation
...
- **Provider configs**: PENNULL_LLM_SERVER_CONFIG_PATH (host path), PENNULL_OLLAMA_SERVER_CONFIG_PATH (host path)
- **Monitoring**: LANGFUSE_BASE_URL, LANGFUSE_PROJECT_ID, OTEL_HOST
- **Docker config**: DOCKER_HOST, PENNULL_DOCKER_CERT_PATH (host path), DOCKER_TLS_VERIFY, DOCKER_CERT_PATH (container path, managed)
- **Deployment modes**: envs determine embedded vs external vs disabled
- **Worker images**: DOCKER_DEFAULT_IMAGE (debian:latest), DOCKER_DEFAULT_IMAGE_FOR_PENTEST (nullailab/kali-linux)
- **Path migration**: DoMigrateSettings() migrates old DOCKER_CERT_PATH/LLM_SERVER_CONFIG_PATH/OLLAMA_SERVER_CONFIG_PATH to PENNULL_* variants on startup
...
- HTTP client for installer updates from pennull.com

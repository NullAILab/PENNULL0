...
The processor package manages penNULL stack lifecycle through state-oriented approach. Main idea: maintain consistency between desired user state (state.State) and actual system state (checker.CheckResult).

## Key Principles

1. **State-Driven Operations**: All operations based on comparing current and target state
2. **Stack Independence**: Each stack (observability, langfuse, pennull) managed independently
...
#### Phase 3: penNULL Stack Management
```go
// penNULL always embedded, always required
...
**Rationale**: penNULL - main stack, always installed, only file integrity check.
...
- dependency ordering: observability → langfuse → pennull

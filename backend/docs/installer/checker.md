...
The `checker` package is responsible for gathering system facts and verifying installation prerequisites for penNULL. It performs comprehensive system analysis to determine the current state of the installation and what operations are available.
...
- Installation status for each component (penNULL, Langfuse, Observability)
...
- **Script Installation**: Verifies penNULL CLI script in /usr/local/bin
...
- Checks updates for: Installer, penNULL, Langfuse, Observability, Worker images

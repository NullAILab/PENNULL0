...
- DOCKER_CERT_PATH migrated to PENNULL_DOCKER_CERT_PATH (host path) + DOCKER_CERT_PATH set to /opt/pennull/docker/ssl (container path)
...
export DOCKER_CERT_PATH=/path/to/certs  # auto-migrated to PENNULL_DOCKER_CERT_PATH on startup
```

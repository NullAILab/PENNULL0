...
import (
	...
	"github.com/nullailab/langchaingo/llms"
	"github.com/nullailab/langchaingo/llms/reasoning"
	"github.com/nullailab/langchaingo/llms/streaming"
)
...
func (fp *flowProvider) storeAgentResponseToGraphiti(
	...
	parts := []string{fmt.Sprintf("penNULL %s agent execution in flow %d", agentType, fp.flowID)}
	if taskID != nil {
...
func (fp *flowProvider) storeToolExecutionToGraphiti(
	...
	parts := []string{fmt.Sprintf("penNULL tool execution in flow %d", fp.flowID)}
	if taskID != nil {
		parts = append(parts, fmt.Sprintf("task %d", *taskID))
	}
	if subtaskID != nil {

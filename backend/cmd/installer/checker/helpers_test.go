...
func TestCheckDiskSpaceWithContext(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:              "pennull not installed",
			workerImageExists: true,
			pennullInstalled:  false,
...
func TestCheckUpdatesServer(t *testing.T) {
	...
	t.Run("successful_response", func(t *testing.T) {
		...
		if response.PentagiIsUpToDate {
			t.Error("expected pennull to not be up to date")
		}
	...
	t.Run("with_proxy", func(t *testing.T) {
		...
		proxyTs := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			fmt.Fprintf(w, `{"installer_is_up_to_date": true, "pennull_is_up_to_date": true, "langfuse_is_up_to_date": true, "observability_is_up_to_date": true}`)
		}))
		defer proxyTs.Close()
...
func TestCheckVolumesExist(t *testing.T) {
	...
	// 1. Exact match: "pennull-data" matches "pennull-data"
	// 2. Compose prefix match: "pennull-data" matches "pennull_pennull-data"
	// 3. Compose prefix match: "pennull-postgres-data" matches "myproject_pennull-postgres-data"
	//
...
func TestCheckVolumesExist_MatchingLogic(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "exact match",
			existingVolumes: []string{"pennull-data", "other-volume"},
			searchVolumes:   []string{"pennull-data"},
			expected:        true,
			description:     "should match exact volume name",
		},
		{
			name:            "compose prefix match",
			existingVolumes: []string{"pennull_pennull-data", "pennull_pennull-ssl"},
			searchVolumes:   []string{"pennull-data"},
			expected:        true,
		...
		},
		{
			...
			existingVolumes: []string{"myproject_pennull-postgres-data", "other_volume"},
			searchVolumes:   []string{"pennull-postgres-data"},
			expected:        true,
		...
		},
		{
			...
			existingVolumes: []string{"pennull-data-backup", "my-pennull-data"},
			searchVolumes:   []string{"pennull-data"},
			expected:        false,
		...
		},
		{
			...
			existingVolumes: []string{"proj_pennull-data", "langfuse-data"},
			searchVolumes:   []string{"pennull-data", "langfuse-data", "missing-volume"},
			expected:        true,
		...
		},
		{
			...
			existingVolumes: []string{"proj1_vol1", "proj2_vol2", "pennull_pennull-ssl"},
			searchVolumes:   []string{"pennull-ssl"},
			expected:        true,

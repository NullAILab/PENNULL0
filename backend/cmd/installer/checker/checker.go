...
var (
	...
	UserAgent        = "penNULL-Installer/" + InstallerVersion
)
...
const (
	...
	PentagiScriptFile            = "/usr/local/bin/pennull"
	PentagiContainerName         = "pennull"
	...
	DefaultImageForPentest       = "nullailab/kali-linux"
	...
	DefaultUpdateServerEndpoint  = "https://update.pennull.com"
...
type CheckResult struct {
	...
	PentagiScriptInstalled  bool   `json:"pennull_script_installed" yaml:"pennull_script_installed"`
	PentagiExtracted        bool   `json:"pennull_extracted" yaml:"pennull_extracted"`
	PentagiInstalled        bool   `json:"pennull_installed" yaml:"pennull_installed"`
	PentagiRunning          bool   `json:"pennull_running" yaml:"pennull_running"`
	PentagiVolumesExist     bool   `json:"pennull_volumes_exist" yaml:"pennull_volumes_exist"`
	...
	PentagiIsUpToDate       bool   `json:"pennull_is_up_to_date" yaml:"pennull_is_up_to_date"`
...
// CanResetPassword returns true when penNULL is running
...
func (h *defaultCheckHandler) GatherWorkerInfo(ctx context.Context, c *CheckResult) error {
	...
	dockerCertPath := getEnvVar(h.appState, "PENNULL_DOCKER_CERT_PATH", "")
...
func (h *defaultCheckHandler) GatherPentagiInfo(ctx context.Context, c *CheckResult) error {
	...
	if h.dockerClient != nil {
		...
		// check if pennull-related volumes exist (indicates previous installation)
		pennullVolumes := []string{"pennull-postgres-data", "pennull-data", "pennull-ssl", "scraper-ssl"}
		c.PentagiVolumesExist = checkVolumesExist(ctx, h.dockerClient, pennullVolumes)
	}
...
func (h *defaultCheckHandler) GatherUpdatesInfo(ctx context.Context, c *CheckResult) error {
	...
	// get penNULL container image info

...
type CheckUpdatesRequest struct {
	...
	PentagiImageName        *string `json:"pennull_image_name,omitempty"`
	PentagiImageTag         *string `json:"pennull_image_tag,omitempty"`
	PentagiImageHash        *string `json:"pennull_image_hash,omitempty"`
...
type CheckUpdatesResponse struct {
	...
	PentagiIsUpToDate       bool `json:"pennull_is_up_to_date"`
...
func checkDirIsWritable(dirPath string) bool {
	...
	tempFile, err := os.CreateTemp(dirPath, ".pennull_test_*")
	if err != nil {
		return false
	}
...
// it matches both exact names and volumes with compose project prefix (e.g., "pennull_pennull-data")
...
func checkDiskSpaceWithContext(
	ctx context.Context,
	workerImageExists, pennullInstalled,
	graphitiConnected, graphitiExternal, graphitiInstalled,
	langfuseConnected, langfuseExternal, langfuseInstalled,
	obsConnected, obsExternal, obsInstalled bool,
) bool {

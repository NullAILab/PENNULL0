...
func TestUpdate(t *testing.T) {
	...
	t.Run("compose_stacks", func(t *testing.T) {
		...
		// Check compose calls - should update langfuse and observability, skip pennull
		composeCalls := composeOps.getCalls()
		updateCount := 0
		for _, call := range composeCalls {
			if call.Method == "updateStack" {
				updateCount++
				// Verify we don't update pennull
				if call.Stack == ProductStackPentagi {
					t.Error("should not update pennull when it's up to date")
				}
			}
		}
		if updateCount != 2 {
	...
	t.Run("all_stacks", func(t *testing.T) {
		...
		// Check compose calls - should update pennull and observability, skip langfuse
		composeCalls := composeOps.getCalls()
		updateCount := 0
		for _, call := range composeCalls {
			if call.Method == "updateStack" {
...
func TestPurge_StrictAndDockerCleanup(t *testing.T) {
	t.Run("compose_stack_purge", func(t *testing.T) {
		...
		if len(composeCalls) == 0 || composeCalls[0].Method != "purgeImagesStack" || composeCalls[0].Stack != ProductStackPentagi {
			t.Fatalf("expected purgeImagesStack call for pennull, got: %+v", composeCalls)
		}
...
func TestApplyChanges_Embedded_AllStacksUpdated(t *testing.T) {
	...
	// since all not extracted -> ensure called for obs, langfuse, graphiti, pennull
...
func TestValidateOperation_ErrorCases(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid start pennull", ProductStackPentagi, ProcessorOperationStart, false, ""},
...
func TestIsEmbeddedDeployment(t *testing.T) {
	tests := []struct {
		...
		{"pennull always embedded", ProductStackPentagi, "", "", false, false, true},     // pennull is always embedded
	...
	tests := []struct {
	...
	}{
		{
			...
			setupError: func(p *processor) {
				_ = p.state.SetVar("OTEL_HOST", checker.DefaultObservabilityEndpoint)
				_ = p.state.SetVar("PENNULL_VERSION", version.GetBinaryVersion()) // make state dirty
		...
		{
			name: "pennull phase error",
			configSetup: func(config *mockCheckConfig) {
				config.PentagiExtracted = false
			},
			setupError: func(p *processor) {
				// make state dirty so applyChanges proceeds
				_ = p.state.SetVar("PENNULL_VERSION", version.GetBinaryVersion())
				injectFSError(p, map[string]error{
					"ensureStackIntegrity_pennull": fmt.Errorf("pennull error"),
					"ensureStackIntegrity":         fmt.Errorf("general error"), // fallback to catch any call
				})
			},
			expectedError: "failed to apply pennull changes: failed to ensure pennull integrity: pennull error",
		},
	}
...
func TestInstall_FullScenario(t *testing.T) {
	t.Run("all_stacks_fresh_install", func(t *testing.T) {
		...
		// should be 3 (observability, langfuse, graphiti) since pennull might be handled differently
		if ensureCount < 3 {
		...
		// all 4 stacks should be updated (observability, langfuse, graphiti, pennull)
		if updateCount != 4 {
	...
	t.Run("partial_install_skip_installed", func(t *testing.T) {
		...
		// should not update pennull since it's already installed
		composeCalls := composeOps.getCalls()
		for _, call := range composeCalls {
			if call.Method == "updateStack" && call.Stack == ProductStackPentagi {
				t.Error("should not update pennull when already installed")
			}
		}
	})
}
...
func TestPreviewFilesStatus_Behavior(t *testing.T) {
	...
	// pennull, observability compose must be present and reflect modified
...
func TestApplyChanges_ComplexScenarios(t *testing.T) {
	t.Run("mixed_deployment_modes", func(t *testing.T) {
		p, composeOps, _, _ := newProcessorForLogicTestsWithConfig(t, func(config *mockCheckConfig) {
			// observability external, langfuse embedded, graphiti disabled, pennull always embedded

...
const (
	...
	pennullExampleCustomConfigLLM = "example.custom.provider.yml"
	pennullExampleOllamaConfigLLM = "example.ollama.provider.yml"
)

var filesToExcludeFromVerification = []string{
	"observability/otel/config.yml",
	"observability/grafana/config/grafana.ini",
	pennullExampleCustomConfigLLM,
	pennullExampleOllamaConfigLLM,
}
...
func (fs *fileSystemOperationsImpl) ensureStackIntegrity(ctx context.Context, stack ProductStack, state *operationState) error {
	...
	case ProductStackPentagi:
		...
		errCustom := fs.ensureFileFromEmbed(pennullExampleCustomConfigLLM, state)
		errOllama := fs.ensureFileFromEmbed(pennullExampleOllamaConfigLLM, state)
...
func (fs *fileSystemOperationsImpl) cleanupStackFiles(ctx context.Context, stack ProductStack, state *operationState) error {
	...
	case ProductStackPentagi:
		...
		filesToRemove = append(filesToRemove, filepath.Join(workingDir, pennullExampleCustomConfigLLM))
		filesToRemove = append(filesToRemove, filepath.Join(workingDir, pennullExampleOllamaConfigLLM))

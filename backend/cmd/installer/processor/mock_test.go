...
func newMockState() *mockState {
	dir, err := os.MkdirTemp("", "pennull-test")
	if err != nil {
		panic(err)
	}
...
func testState(t *testing.T) *mockState {
	...
	if _, err := os.Stat(envPath); os.IsNotExist(err) {
		if err := os.WriteFile(envPath, []byte("PENNULL_VERSION=1.0.0\n"), 0o644); err != nil {
...
func TestMockOperations_ErrorIsolation(t *testing.T) {
	t.Run("StackSpecificErrors", func(t *testing.T) {
		...
		mock.errOn["ensureStackIntegrity_"+string(ProductStackPentagi)] = fmt.Errorf("pennull-specific error")

		// pennull should fail
		err := mock.ensureStackIntegrity(t.Context(), ProductStackPentagi, state)
		if err == nil || err.Error() != "pennull-specific error" {
			t.Errorf("expected pennull-specific error, got %v", err)
		}
...
func TestMockState_ComplexOperations(t *testing.T) {
	...
	t.Run("StackManagement", func(t *testing.T) {
		stack := []string{"pennull", "langfuse", "observability"}
...
func TestMockCheckHandler_CompleteScenarios(t *testing.T) {
	...
	t.Run("PartialFailures", func(t *testing.T) {
		...
		if result.PentagiInstalled {
			t.Error("expected penNULL not to be installed")
		}

...
const (
	...
	WelcomeFormTitle       = "Welcome to penNULL"
	WelcomeFormDescription = "penNULL is an autonomous penetration testing platform that leverages AI technologies to perform comprehensive security assessments."
...
The installer guides you through each component setup with recommendations for different deployment scenarios.`
	...
	WelcomeWorkflowStep5 = "5. Deploy and start penNULL services"
...
const (
	...
	TroubleshootEnvFileDesc  = "The .env file is required for penNULL configuration but was not found or is not readable."
...
4. Ensure sufficient disk space is available`
	...
	TroubleshootDockerNotInstalledDesc  = "Docker is not installed on this system. penNULL requires Docker to run containers."
...
5. Try: export DOCKER_HOST=unix:///var/run/docker.sock`
	...
	TroubleshootDockerVersionDesc  = "Your Docker version is incompatible. penNULL requires Docker 20.0.0 or newer."
...
Verify installation: docker compose version`
	...
	TroubleshootComposeVersionDesc  = "Your Docker Compose version is incompatible. penNULL requires Docker Compose 1.25.0 or newer."
...
5. Ensure certificates are valid if using TLS`
	...
	TroubleshootCPUDesc  = "penNULL requires at least 2 CPU cores for proper operation."
...
const (
	// Form interface implementation
	EULAFormDescription = "Legal terms and conditions for penNULL usage"
	EULAFormName        = "EULA"
	EULAFormOverview    = `Review and accept the End User License Agreement to proceed with penNULL installation.
...
const (
	MainMenuTitle       = "penNULL Configuration"
	MainMenuDescription = "Configure all penNULL components and settings"
	MainMenuName        = "Main Menu"
	MainMenuOverview    = `Welcome to penNULL Configuration Center.
...
Navigate through each section to complete your penNULL setup.`
...
const (
	MainMenuStatusPentagiRunning     = "penNULL is already running"
	MainMenuStatusPentagiNotRunning  = "Ready to start penNULL services"
	MainMenuStatusUpToDate           = "penNULL is up to date"
...
const (
	...
	LLMProvidersOverview    = `penNULL uses specialized AI agents (researcher, developer, executor, pentester) that require different LLM capabilities for optimal penetration testing results.
...
Ready-to-use configurations for OpenRouter, DeepInfra, vLLM, Ollama, and other providers are available in the /opt/pennull/conf/ directory inside the container`
)
...
Examples available: Pre-configured setups for major providers in /opt/pennull/conf/ directory inside the container`
)
...
Monitor penNULL performance and system health.`
...
const (
	...
	ServerSettingsFormDescription = "Configure penNULL server network access and public routing"
	ServerSettingsFormName        = "Server Settings"
	ServerSettingsFormOverview    = `• Network binding - control which interface and port penNULL listens on
...
• Data directory - persistent storage for agent artifacts and flow workspaces`
	...
	ServerSettingsLicenseKeyDesc = "penNULL License Key in format of XXXX-XXXX-XXXX-XXXX"
	...
	ServerSettingsPortDesc = "External TCP port exposed by Docker for penNULL web UI"

	ServerSettingsPublicURL     = "Public URL"
	ServerSettingsPublicURLDesc = "Base public URL for redirects and links (supports base path, e.g., https://example.com/pennull/)"
	...
	ServerSettingsExternalSSLCAPathDesc = "Path inside container to custom root CA cert (e.g., /opt/pennull/ssl/ca-bundle.pem)"
	...
	ServerSettingsGeneralHelp = `penNULL exposes its web UI via Docker with configurable host and port.

Public URL must reflect how users reach the server. If using a subpath (e.g., /pennull/), include it here. CORS controls browser access from specified origins. Proxy affects outbound traffic to LLM/search providers and other external services used by Tools.
...
SSL directory allows providing custom certificates. When set, server will use server.crt and server.key from that directory. Data directory stores artifacts and working files for flows.`
	...
	ServerSettingsLicenseKeyHelp = `penNULL License Key in format of XXXX-XXXX-XXXX-XXXX. It's used to communicate with penNULL Cloud API.`
...
• 0.0.0.0 — expose on all interfaces`
	...
	ServerSettingsPortHelp = `External port for penNULL UI. Must be available on the host. Example: 8443.`
...
• http://localhost:8443
• https://example.com/
• https://example.com/pennull/ (with base path)`
...
Must point to /opt/pennull/ssl/ directory, which is mounted from pennull-ssl volume on the host.

Examples:
• /opt/pennull/ssl/ca-bundle.pem
• /opt/pennull/ssl/corporate-ca.pem
...
When enabled, all certificate validation is bypassed, making connections vulnerable to man-in-the-middle attacks.`
	...
	ServerSettingsDataDirHelp = `Host directory for persistent data. penNULL stores agent artifacts under flow-N subdirectories, which map to /work inside worker containers.`
...
const (
	...
	ToolsAIAgentsSettingsFormOverview    = `This section configures global behavior of AI agents across penNULL.
...
const (
	...
	ToolsScraperFormOverview    = `Web scraper service for content extraction and analysis using nullailab/scraper Docker image.
...
Docker image: https://hub.docker.com/r/nullailab/scraper
...
Examples:
• 'pennull-network' - creates isolated bridge network
...
Recommended images include Kali Linux, Parrot Security, or custom security-focused containers. Default: nullailab/kali-linux`
...
Change providers carefully - different embedders produce incompatible vectors requiring database reindexing.`
	...
	EmbedderHelpGeneral = `Embeddings convert text into vectors for semantic search and knowledge storage. This enables penNULL to understand meaning rather than just keywords, making search results more relevant and intelligent.
...
const (
	...
	ApplyChangesFormOverview = `This screen allows you to review all pending configuration changes and apply them to your penNULL installation.
...
• Install additional components if needed`
	...
	ApplyChangesInstallNotFound = `penNULL is not currently installed on this system.

The following actions will be performed:
• Docker environment setup and validation
• Creation of docker-compose.yml file
• Installation and startup of penNULL core services`
	...
	ApplyChangesUpdateFound = `penNULL is currently installed on this system.
...
const (
	...
	MaintenanceDescription = "Manage penNULL services and perform maintenance operations"
	MaintenanceName        = "Maintenance"
	MaintenanceOverview    = `Perform system maintenance operations for penNULL.
...
Each operation will provide real-time status updates and confirmation when required.`
	...
	MaintenanceStartPentagi            = "Start penNULL"
	MaintenanceStartPentagiDesc        = "Start all configured penNULL services"
	MaintenanceStopPentagi             = "Stop penNULL"
	MaintenanceStopPentagiDesc         = "Stop all running penNULL services"
	MaintenanceRestartPentagi          = "Restart penNULL"
	MaintenanceRestartPentagiDesc      = "Restart all penNULL services"
	...
	MaintenanceUpdatePentagi           = "Update penNULL"
	MaintenanceUpdatePentagiDesc       = "Update penNULL to the latest version"
	...
	MaintenanceFactoryResetDesc        = "Reset penNULL to factory defaults"
	MaintenanceRemovePentagi           = "Remove penNULL"
	MaintenanceRemovePentagiDesc       = "Remove penNULL containers but keep data"
	MaintenancePurgePentagi            = "Purge penNULL"
	MaintenancePurgePentagiDesc        = "Completely remove penNULL including all data"
	MaintenanceResetPassword           = "Reset Admin Password"
	MaintenanceResetPasswordDesc       = "Reset the administrator password for penNULL"
)

// Reset Password Screen constants
const (
	ResetPasswordFormTitle       = "Reset Admin Password"
	ResetPasswordFormDescription = "Reset the administrator password for penNULL"
	ResetPasswordFormName        = "Reset Password"
	ResetPasswordFormOverview    = `Reset the password for the default administrator account (admin@pennull.com).

This operation requires penNULL to be running and will update the password in the PostgreSQL database.
...
• Both password fields must match`
	...
	// Status messages
	ResetPasswordNotAvailable = "penNULL must be running to reset password"
	...
	// Help content
	ResetPasswordHelpContent = `Reset the administrator password for accessing penNULL.

This operation:
• Updates the password for admin@pennull.com account
• Sets the user status to 'active'
• Requires penNULL database to be accessible
...
const (
	...
	ProcessorOperationInstalling  = "Installing penNULL services..."
	...
	OperationTitleInstallPentagi    = "Install penNULL"
	OperationDescInstallPentagi     = "Install and configure penNULL services"
...
• Core penNULL API and web interface
...
const (
	...
	EnvDesc_LICENSE_KEY                       = "penNULL License Key"
	EnvDesc_PENNULL_LISTEN_IP                 = "penNULL Server Host"
	EnvDesc_PENNULL_LISTEN_PORT               = "penNULL Server Port"
	EnvDesc_PUBLIC_URL                        = "penNULL Public URL"
	EnvDesc_CORS_ORIGINS                      = "penNULL CORS Origins"
	EnvDesc_COOKIE_SIGNING_SALT               = "penNULL Cookie Signing Salt"
	...
	EnvDesc_PENNULL_SSL_DIR                   = "penNULL SSL Directory"
	EnvDesc_PENNULL_DATA_DIR                  = "penNULL Data Directory"
	EnvDesc_PENNULL_DOCKER_SOCKET             = "Mount Docker Socket Path"
	EnvDesc_PENNULL_DOCKER_CERT_PATH          = "Mount Docker Certificate Path"
	EnvDesc_PENNULL_LLM_SERVER_CONFIG_PATH    = "Custom LLM Host Config Path"
	EnvDesc_PENNULL_OLLAMA_SERVER_CONFIG_PATH = "Ollama Host Config Path"
	...
	EnvDesc_PENNULL_POSTGRES_PASSWORD = "penNULL PostgreSQL Password"
...
const (
	...
	// component labels
	ProcessorComponentPentagi       = "penNULL"
	...
	EffectsStart           = "penNULL web UI becomes available. Background services are brought online in the required order."
	EffectsStop            = "Web UI becomes unavailable. In-progress flows pause safely. When you start penNULL again, flows resume automatically. A small portion of the current agent step may be lost."

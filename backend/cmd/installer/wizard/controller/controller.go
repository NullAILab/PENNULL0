...
import (
	...
	"pennull/cmd/installer/checker"
	"pennull/cmd/installer/files"
	"pennull/cmd/installer/loader"
	"pennull/cmd/installer/state"
	"pennull/cmd/installer/wizard/locale"
)

const (
	EmbeddedLLMConfigsPath   = "providers-configs"
	DefaultDockerCertPath    = "/opt/pennull/docker/ssl"
	DefaultCustomConfigsPath = "/opt/pennull/conf/custom.provider.yml"
	DefaultOllamaConfigsPath = "/opt/pennull/conf/ollama.provider.yml"
	DefaultLLMConfigsPath    = "/opt/pennull/conf/"
...
type LLMProviderConfig struct {
	...
	HostConfigPath    loader.EnvVar // PENNULL_OLLAMA_SERVER_CONFIG_PATH | PENNULL_LLM_SERVER_CONFIG_PATH
...
func (c *controller) GetLLMProviderConfig(providerID string) *LLMProviderConfig {
	...
	case "ollama":
		...
		providerConfig.HostConfigPath, _ = c.GetVar("PENNULL_OLLAMA_SERVER_CONFIG_PATH")
	...
	case "custom":
		...
		providerConfig.HostConfigPath, _ = c.GetVar("PENNULL_LLM_SERVER_CONFIG_PATH")
		if slices.Contains(providersConfigsPath, providerConfig.ConfigPath.Value) {
			providerConfig.HostConfigPath.Value = providerConfig.ConfigPath.Value
		}
...
func (c *controller) ResetLLMProviderConfig(providerID string) map[string]*LLMProviderConfig {
	...
	case "ollama":
		vars = []string{
			...
			"PENNULL_OLLAMA_SERVER_CONFIG_PATH",
		}
	case "deepseek":
	...
	case "custom":
		vars = []string{
			...
			"PENNULL_LLM_SERVER_CONFIG_PATH", // local path to the LLM config file
		}
	}
...
type DockerConfig struct {
	...
	HostDockerCertPath loader.EnvVar // PENNULL_DOCKER_CERT_PATH
...
func (c *controller) GetDockerConfig() *DockerConfig {
	...
	config := &DockerConfig{
		...
		HostDockerCertPath:           vars["PENNULL_DOCKER_CERT_PATH"],
	}
...
func (c *controller) UpdateDockerConfig(config *DockerConfig) error {
	updates := map[string]string{
		...
		"PENNULL_DOCKER_CERT_PATH":         config.HostDockerCertPath.Value,
	}

	dockerHost := config.DockerHost.Value
	if strings.HasPrefix(dockerHost, "unix://") && !config.DockerHost.IsDefault() {
		// mount custom docker socket to the pennull container
		updates["PENNULL_DOCKER_SOCKET"] = strings.TrimPrefix(dockerHost, "unix://")
	} else {
		// ensure previous custom socket mapping is cleared when not using unix socket
		updates["PENNULL_DOCKER_SOCKET"] = ""
	}
...
// ServerSettingsConfig represents penNULL server settings configuration
type ServerSettingsConfig struct {
	// direct form field mappings using loader.EnvVar
	LicenseKey          loader.EnvVar // LICENSE_KEY
	ListenIP            loader.EnvVar // PENNULL_LISTEN_IP
	ListenPort          loader.EnvVar // PENNULL_LISTEN_PORT
	...
	SSLDir              loader.EnvVar // PENNULL_SSL_DIR
	DataDir             loader.EnvVar // PENNULL_DATA_DIR
...
func (c *controller) GetServerSettingsConfig() *ServerSettingsConfig {
	...
	defaults := map[string]string{
		"LICENSE_KEY":           "",
		"PENNULL_LISTEN_IP":     "127.0.0.1",
		"PENNULL_LISTEN_PORT":   "8443",
		"PUBLIC_URL":            "https://localhost:8443",
		"CORS_ORIGINS":          "https://localhost:8443",
		"PENNULL_DATA_DIR":      "pennull-data",
		"PENNULL_SSL_DIR":       "pennull-ssl",
	...
	cfg := &ServerSettingsConfig{
		...
		ListenIP:            vars["PENNULL_LISTEN_IP"],
		ListenPort:          vars["PENNULL_LISTEN_PORT"],
		...
		SSLDir:              vars["PENNULL_SSL_DIR"],
		DataDir:             vars["PENNULL_DATA_DIR"],
	}
...
func (c *controller) UpdateServerSettingsConfig(config *ServerSettingsConfig) error {
	...
	updates := map[string]string{
		...
		"PENNULL_LISTEN_IP":     config.ListenIP.Value,
		"PENNULL_LISTEN_PORT":   config.ListenPort.Value,
		...
		"PENNULL_SSL_DIR":       config.SSLDir.Value,
		"PENNULL_DATA_DIR":      config.DataDir.Value,
	}
...
type ApplyChangesConfig struct {
	// installation state
	IsInstalled bool // whether penNULL is currently installed
...
func (c *controller) getVariableDescription(varName string) string {
	...
	envVarDescriptions := map[string]string{
		...
		"PENNULL_LISTEN_IP":                 locale.EnvDesc_PENNULL_LISTEN_IP,
		"PENNULL_LISTEN_PORT":               locale.EnvDesc_PENNULL_LISTEN_PORT,
		...
		"PENNULL_SSL_DIR":                   locale.EnvDesc_PENNULL_SSL_DIR,
		"PENNULL_DATA_DIR":                  locale.EnvDesc_PENNULL_DATA_DIR,
		"PENNULL_DOCKER_SOCKET":             locale.EnvDesc_PENNULL_DOCKER_SOCKET,
		"PENNULL_DOCKER_CERT_PATH":          locale.EnvDesc_PENNULL_DOCKER_CERT_PATH,
		"PENNULL_LLM_SERVER_CONFIG_PATH":    locale.EnvDesc_PENNULL_LLM_SERVER_CONFIG_PATH,
		"PENNULL_OLLAMA_SERVER_CONFIG_PATH": locale.EnvDesc_PENNULL_OLLAMA_SERVER_CONFIG_PATH,
		...
		"PENNULL_POSTGRES_PASSWORD": locale.EnvDesc_PENNULL_POSTGRES_PASSWORD,
...
var maskedVariables = map[string]bool{
	...
	// postgres password for pennull service (pgvector binds on localhost)
	"PENNULL_POSTGRES_PASSWORD": true,
...
var criticalVariables = map[string]bool{
	...
	// mounting custom LLM server config into pennull container changes volume mapping
	"PENNULL_LLM_SERVER_CONFIG_PATH":    true,
	"PENNULL_OLLAMA_SERVER_CONFIG_PATH": true,
	...
	"LICENSE_KEY":           true,
	"PENNULL_LISTEN_IP":     true,
	"PENNULL_LISTEN_PORT":   true,
	"PUBLIC_URL":            true,
	"CORS_ORIGINS":          true,
	"COOKIE_SIGNING_SALT":   true,
	"PROXY_URL":             true,
	...
	"SERVER_SSL_KEY":        true,
	"SERVER_USE_SSL":        true,
	"PENNULL_SSL_DIR":       true,
	"PENNULL_DATA_DIR":      true,
	...
	// langfuse integration settings passed to pennull

...
import (
	...
	"pennull/cmd/installer/wizard/controller"
)
...
func TestDoMigrateSettings_SuccessfulMigrations(t *testing.T) {
	tests := []struct {
		...
		{
			name: "migrate DOCKER_CERT_PATH to PENNULL_DOCKER_CERT_PATH",
			...
			pennullVarName:  "PENNULL_DOCKER_CERT_PATH",
			defaultPath:     controller.DefaultDockerCertPath,
			pathType:        directory,
			expectMigration: true,
		},
		{
			name: "migrate LLM_SERVER_CONFIG_PATH to PENNULL_LLM_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (string, func()) {
			...
			pennullVarName:  "PENNULL_LLM_SERVER_CONFIG_PATH",
			defaultPath:     controller.DefaultCustomConfigsPath,
			pathType:        file,
			expectMigration: true,
		},
		{
			name: "migrate OLLAMA_SERVER_CONFIG_PATH to PENNULL_OLLAMA_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (string, func()) {
			...
			pennullVarName:  "PENNULL_OLLAMA_SERVER_CONFIG_PATH",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectMigration {
				// check that PENNULL_* variable was set to custom path
				pennullVar, exists := mockSt.GetVar(tt.pennullVarName)
				if !exists {
					t.Errorf("Expected %s to be set", tt.pennullVarName)
				} else if pennullVar.Value != customPath {
					t.Errorf("Expected %s = %q, got %q", tt.pennullVarName, customPath, pennullVar.Value)
				}
...
func TestDoMigrateSettings_VariableNotSet(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.pennullVarName)
			if exists {
				t.Errorf("Expected %s to not be set", tt.pennullVarName)
			}
		})
	}
}
...
func TestDoMigrateSettings_PathNotExist(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.pennullVarName)
			if exists {
				t.Errorf("Expected %s to not be set for non-existing path", tt.pennullVarName)
			}
		})
	}
}
...
func TestDoMigrateSettings_AlreadyDefaultValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.pennullVarName)
			if exists {
				t.Errorf("Expected %s to not be set when already using default", tt.pennullVarName)
			}
...
func TestDoMigrateSettings_EmbeddedConfigs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			varName:        "LLM_SERVER_CONFIG_PATH",
			pennullVarName: "PENNULL_LLM_SERVER_CONFIG_PATH",
			embeddedPath:   "/opt/pennull/conf/llms/openai.yml",
		...
		},
		{
			...
			varName:        "OLLAMA_SERVER_CONFIG_PATH",
			pennullVarName: "PENNULL_OLLAMA_SERVER_CONFIG_PATH",
			embeddedPath:   "/opt/pennull/conf/llms/llama3.yml",
			description:    "Embedded configs are inside docker image, no migration needed",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// verify no migration occurred
			pennullVar, exists := mockSt.GetVar(tt.pennullVarName)
			if exists && pennullVar.Value != "" {
				t.Errorf("Expected %s to not be set for embedded config: %s", tt.pennullVarName, tt.description)
			}
...
func TestDoMigrateSettings_WrongPathType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, exists := mockSt.GetVar(tt.pennullVarName)
			if exists {
				t.Errorf("Expected %s to not be set for wrong path type: %s", tt.pennullVarName, tt.description)
			}
		})
	}
}
...
func TestDoMigrateSettings_ErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			name: "SetVar error for PENNULL_DOCKER_CERT_PATH",
			setupFunc: func(t *testing.T) (*mockStateWithErrors, string, func()) {
				...
				mockSt := &mockStateWithErrors{
					...
					setVarError: map[string]error{
						"PENNULL_DOCKER_CERT_PATH": mockError,
					},
				}
		...
		{
			name: "SetVar error for PENNULL_LLM_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (*mockStateWithErrors, string, func()) {
				tmpFile, err := os.CreateTemp("", "custom-*.yml")
				if err != nil {
					t.Fatalf("Failed to create temp file: %v", err)
				...
				mockSt := &mockStateWithErrors{
					...
					setVarError: map[string]error{
						"PENNULL_LLM_SERVER_CONFIG_PATH": mockError,
					},
				}
		...
		{
			name: "SetVar error for PENNULL_OLLAMA_SERVER_CONFIG_PATH",
			setupFunc: func(t *testing.T) (*mockStateWithErrors, string, func()) {
				tmpFile, err := os.CreateTemp("", "ollama-*.yml")
				if err != nil {
					t.Fatalf("Failed to create temp file: %v", err)
				...
				mockSt := &mockStateWithErrors{
					...
					setVarError: map[string]error{
						"PENNULL_OLLAMA_SERVER_CONFIG_PATH": mockError,
					},
				}
				return mockSt, tmpFile.Name(), func() { os.Remove(tmpFile.Name()) }
			},
...
func TestDoMigrateSettings_CombinedMigrations(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedVars: map[string]string{
				...
				// PENNULL_* vars will be checked separately as they contain dynamic temp paths
			},
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (map[string]string, func()) {
				...
				paths := map[string]string{
					"DOCKER_CERT_PATH":          dockerCertDir,
					"LLM_SERVER_CONFIG_PATH":    "/opt/pennull/conf/llms/openai.yml", // embedded config
					"OLLAMA_SERVER_CONFIG_PATH": "/opt/pennull/conf/llms/llama3.yml", // embedded config
				}
			...
			expectedVars: map[string]string{
				"DOCKER_CERT_PATH":          controller.DefaultDockerCertPath,
				"LLM_SERVER_CONFIG_PATH":    "/opt/pennull/conf/llms/openai.yml", // should not change
				"OLLAMA_SERVER_CONFIG_PATH": "/opt/pennull/conf/llms/llama3.yml", // should not change
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// verify PENNULL_* variables were set correctly for non-default and non-embedded values
			for varName, originalValue := range paths {
				...
				case "LLM_SERVER_CONFIG_PATH":
					...
					isEmbedded = strings.HasPrefix(originalValue, "/opt/pennull/conf/llms/")
				...
				if shouldMigrate {
					...
					if err == nil {
						...
						pennullVar, exists := mockSt.GetVar(pennullVarName)
						if !exists {
							t.Errorf("Expected %s to be set for non-default value", pennullVarName)
						} else if pennullVar.Value != originalValue {
							t.Errorf("Expected %s = %q, got %q", pennullVarName, originalValue, pennullVar.Value)
						}
					}
				} else {
					...
					pennullVar, exists := mockSt.GetVar(pennullVarName)
					if exists && pennullVar.Value != "" {
						t.Errorf("Expected %s to not be set for default/embedded value, but got %q", pennullVarName, pennullVar.Value)
					}
				}
			}
		})

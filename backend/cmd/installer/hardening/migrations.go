...
func DoMigrateSettings(s state.State) error {
	// migration from DOCKER_CERT_PATH to PENNULL_DOCKER_CERT_PATH
	...
	if exists && dockerCertPath != "" && dockerCertPath != controller.DefaultDockerCertPath {
		if err := s.SetVar("PENNULL_DOCKER_CERT_PATH", dockerCertPath); err != nil {
			return err
		}
	...
	// migration from LLM_SERVER_CONFIG_PATH to PENNULL_LLM_SERVER_CONFIG_PATH
	...
	if exists && !isEmbeddedCustomConfig && llmServerConfigPath != "" {
		if err := s.SetVar("PENNULL_LLM_SERVER_CONFIG_PATH", llmServerConfigPath); err != nil {
			return err
		}
	...
	// migration from OLLAMA_SERVER_CONFIG_PATH to PENNULL_OLLAMA_SERVER_CONFIG_PATH
	...
	if exists && !isEmbeddedOllamaConfig && ollamaServerConfigPath != "" {
		if err := s.SetVar("PENNULL_OLLAMA_SERVER_CONFIG_PATH", ollamaServerConfigPath); err != nil {
			return err
		}

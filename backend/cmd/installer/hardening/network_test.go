...
// Test 6: DOCKER_CERT_PATH migration to PENNULL_DOCKER_CERT_PATH
func TestDoSyncNetworkSettings_DockerCertPathMigration(t *testing.T) {
	tests := []struct {
		...
		{
			name: "DOCKER_CERT_PATH with existing directory should migrate to PENNULL_DOCKER_CERT_PATH",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectMigration {
				// Check PENNULL_DOCKER_CERT_PATH was set to original path
				pennullVar, exists := mockSt.GetVar("PENNULL_DOCKER_CERT_PATH")
				if !exists {
					t.Errorf("Expected PENNULL_DOCKER_CERT_PATH to be set: %s", tt.description)
				} else if pennullVar.Value != dockerCertPath {
					t.Errorf("Expected PENNULL_DOCKER_CERT_PATH = %q, got %q: %s", dockerCertPath, pennullVar.Value, tt.description)
				}
				...
				// Check PENNULL_DOCKER_CERT_PATH was set but empty (migration did not occur)
				pennullVar, exists := mockSt.GetVar("PENNULL_DOCKER_CERT_PATH")
				if !exists {
					t.Errorf("Expected PENNULL_DOCKER_CERT_PATH to exist in state: %s", tt.description)
				} else if pennullVar.Value != "" {
					t.Errorf("Expected PENNULL_DOCKER_CERT_PATH to be empty, got %q: %s", pennullVar.Value, tt.description)
				}
...
func TestDoSyncNetworkSettings_PreventOverrideExistingSettings(t *testing.T) {
	tests := []struct {
		...
		{
			name: "existing PENNULL_DOCKER_CERT_PATH in state prevents sync",
			existingStateVars: map[string]string{
				"PENNULL_DOCKER_CERT_PATH": "/existing/certs",
			},
			envVars: map[string]string{
				"DOCKER_HOST": "tcp://new.example.com:2376",
			},
			expectSync:  false,
			description: "State with PENNULL_DOCKER_CERT_PATH should prevent sync",
		},
		{

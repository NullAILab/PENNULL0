...
**Penetration testing NULL Intelligence** — Autonomous AI-Powered Security Testing Platform

*by [NullAI Lab](https://github.com/NullAILab) · Project 43 of 67*
...
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL+pgvector-16-4169E1?logo=postgresql&logoColor=white)](https://www.postgresql.org/)
...
> **⚠️ Ethical Use Only.** penNULL is designed exclusively for authorized penetration testing, security research, and educational purposes. Always obtain explicit written permission before testing any system. Unauthorized use is illegal and unethical.
...
**penNULL** is a fully autonomous penetration testing platform powered by a multi-agent AI system. It combines the reasoning power of large language models with a professional suite of security tools — running entirely inside isolated Docker containers — to conduct end-to-end penetration tests with minimal human intervention.

Unlike traditional security scanners that run predefined checks, penNULL *thinks*: it reads the target, forms a hypothesis, executes tools, interprets results, adapts its strategy, and delivers a complete vulnerability report — all without manual step-by-step guidance.
...
### What makes penNULL different?

| Capability | Traditional Tools | penNULL |
|------------|-------------------|---------|
| Execution model | Manual, step-by-step | Fully autonomous AI agents |
| Adaptability | Fixed scan profiles | Dynamic strategy based on findings |
| Memory | None | Persistent vector DB + knowledge graph |
| Reporting | Raw output dumps | Structured vulnerability reports |
| LLM support | None | 10+ providers (OpenAI, Claude, Gemini, Ollama, …) |
| Isolation | Host-level | Docker sandboxed containers per task |
| Observability | Basic logs | Grafana + OpenTelemetry + Langfuse |
...
- Web Intelligence. Built-in browser via [scraper](https://hub.docker.com/r/nullailab/scraper) for gathering latest information from web sources.
...
- Persistent Storage. All commands and outputs are stored in PostgreSQL with [pgvector](https://hub.docker.com/r/nullailab/pgvector) extension.
...
penNULL includes sophisticated multi-layered agent supervision mechanisms to ensure efficient task execution, prevent infinite loops, and provide intelligent recovery from stuck states:
...
The architecture of penNULL is designed to be modular, scalable, and secure. Here are the key components:
...
penNULL provides comprehensive programmatic access through both REST and GraphQL APIs, allowing you to integrate penetration testing workflows into your automation pipelines, CI/CD processes, and custom applications.

### Generating API Tokens

API tokens are managed through the penNULL web interface:
...
penNULL provides interactive documentation for exploring and testing API endpoints:

#### GraphQL Playground

Access the GraphQL Playground at `https://your-pennull-instance:8443/api/v1/graphql/playground`
...
Access the REST API documentation at `https://your-pennull-instance:8443/api/v1/swagger/index.html`
...
You can generate type-safe API clients for your preferred programming language using the schema files included with penNULL:
...
penNULL supports Ollama for both local LLM inference (zero-cost, enhanced privacy) and Ollama Cloud (managed service with free tier).
...
The `PENNULL_OLLAMA_SERVER_CONFIG_PATH` environment variable maps your host configuration file to `/opt/pennull/conf/ollama.provider.yml` inside the container.
...
penNULL requires models with larger context windows than the default Ollama configurations. You need to create custom models with increased `num_ctx` parameter through Modelfiles. While typical agent workflows consume around 64K tokens, penNULL uses 110K context size for safety margin and handling complex penetration testing scenarios.
...
These custom models are referenced in the pre-built provider configuration files (`ollama-qwen332b-fp16-tc.provider.yml` and `ollama-qwq32b-fp16-tc.provider.yml`) that are included in the Docker image at `/opt/pennull/conf/`.

### OpenAI Provider Configuration

penNULL integrates with OpenAI's comprehensive model lineup, featuring advanced reasoning capabilities with extended chain-of-thought, agentic models with enhanced tool integration, and specialized code models for security engineering.
...
penNULL supports 31 OpenAI models with tool calling, streaming, reasoning modes, and prompt caching. Models marked with `*` are used in default configuration.
...
>
> All GPT-5 series models (`gpt-5`, `gpt-5.1`, `gpt-5.2`, `gpt-5-pro`, `gpt-5.2-pro`, and all Codex variants) work **unstably with penNULL** and may trigger OpenAI's cybersecurity safety mechanisms without verified access.
>
...
penNULL integrates with Anthropic's Claude models, featuring advanced extended thinking capabilities, exceptional safety mechanisms, and sophisticated understanding of complex security contexts with prompt caching.
...
penNULL supports 10 Claude models with tool calling, streaming, extended thinking, adaptive thinking, and prompt caching. Models marked with `*` are used in default configuration.
...
penNULL integrates with Google's Gemini models through the Google AI API, offering state-of-the-art multimodal reasoning capabilities with extended thinking and context caching.
...
penNULL supports 13 Gemini models with tool calling, streaming, thinking modes, and context caching. Models marked with `*` are used in default configuration.
...
penNULL integrates with Amazon Bedrock, offering access to 20+ foundation models from leading AI companies including Anthropic, Amazon, Cohere, DeepSeek, OpenAI, Qwen, Mistral, and Moonshot.
...
penNULL supports 21 AWS Bedrock models with tool calling, streaming, and multimodal capabilities. Models marked with `*` are used in default configuration.
...
>
> penNULL uses Amazon Bedrock **Converse API** for unified model access. All supported models require:
>
...
penNULL integrates with DeepSeek, providing access to advanced AI models with strong reasoning, coding capabilities, and context caching at competitive prices.
...
penNULL supports 2 DeepSeek-V3.2 models with tool calling, streaming, thinking modes, and context caching. Both models are used in default configuration.
...
**LiteLLM Integration**: Set `DEEPSEEK_PROVIDER=deepseek` to enable model name prefixing when using default penNULL configurations with LiteLLM proxy. Leave empty for direct API usage.

### GLM Provider Configuration

penNULL integrates with GLM from Zhipu AI (Z.AI), providing advanced language models with MoE architecture, strong reasoning, and agentic capabilities developed by Tsinghua University.
...
penNULL supports 12 GLM models with tool calling, streaming, thinking modes, and prompt caching. Models marked with `*` are used in default configuration.
...
**LiteLLM Integration**: Set `GLM_PROVIDER=zai` to enable model name prefixing when using default penNULL configurations with LiteLLM proxy. Leave empty for direct API usage.

### Kimi Provider Configuration

penNULL integrates with Kimi from Moonshot AI, providing ultra-long context models with multimodal capabilities perfect for analyzing extensive codebases and documentation.
...
penNULL supports 11 Kimi/Moonshot models with tool calling, streaming, thinking modes, and multimodal capabilities. Models marked with `*` are used in default configuration.
...
**LiteLLM Integration**: Set `KIMI_PROVIDER=moonshot` to enable model name prefixing when using default penNULL configurations with LiteLLM proxy. Leave empty for direct API usage.

### Qwen Provider Configuration

penNULL integrates with Qwen from Alibaba Cloud Model Studio (DashScope), providing powerful multilingual models with reasoning capabilities and context caching support.
...
penNULL supports 32 Qwen models with tool calling, streaming, thinking modes, and context caching. Models marked with `*` are used in default configuration.
...
**LiteLLM Integration**: Set `QWEN_PROVIDER=dashscope` to enable model name prefixing when using default penNULL configurations with LiteLLM proxy. Leave empty for direct API usage.
...
### API Keys and Tokens
- `LANGFUSE_INIT_PROJECT_PUBLIC_KEY` - Project public key (used from penNULL side too)
- `LANGFUSE_INIT_PROJECT_SECRET_KEY` - Project secret key (used from penNULL side too)
...
2. Enable integration with Langfuse for penNULL service in `.env` file.
...
1. Enable integration with OpenTelemetry and all observability services for penNULL in `.env` file.
...
>
> ```bash
> alias pennull="docker compose -f docker-compose.yml -f docker-compose-langfuse.yml -f docker-compose-graphiti.yml -f docker-compose-observability.yml"
> alias pennull-up="docker compose -f docker-compose.yml -f docker-compose-langfuse.yml -f docker-compose-graphiti.yml -f docker-compose-observability.yml up -d"
> alias pennull-down="docker compose -f docker-compose.yml -f docker-compose-langfuse.yml -f docker-compose-graphiti.yml -f docker-compose-observability.yml down"
> ```

### Knowledge Graph Integration (Graphiti)

penNULL integrates with [Graphiti](https://github.com/nullailab/pennull-graphiti), a temporal knowledge graph system powered by Neo4j, to provide advanced semantic understanding and relationship tracking for AI agent operations. The nullailab fork provides custom entity and edge types that are specific to pentesting purposes.
...
2. Run the Graphiti stack along with the main penNULL services:
...
> [!NOTE]
> The Graphiti service is defined in `docker-compose-graphiti.yml` as a separate stack. You must run both compose files together to enable the knowledge graph functionality. The pre-built Docker image `nullailab/graphiti:latest` is used by default.

#### What Gets Stored

When enabled, penNULL automatically captures:
...
penNULL allows you to configure Docker image selection for executing various tasks. The system automatically chooses the most appropriate image based on the task type, but you can constrain this selection by specifying your preferred images:

| Variable                           | Default                | Description                                                 |
| ---------------------------------- | ---------------------- | ----------------------------------------------------------- |
| `DOCKER_DEFAULT_IMAGE`             | `debian:latest`        | Default Docker image for general tasks and ambiguous cases  |
| `DOCKER_DEFAULT_IMAGE_FOR_PENTEST` | `nullailab/kali-linux` | Default Docker image for security/penetration testing tasks |
...
- `DATABASE_URL` - PostgreSQL database URL (eg. `postgres://postgres:postgres@localhost:5432/pennulldb?sslmode=disable`)
...
- Run `go run cmd/pennull/main.go` to start the server
...
penNULL includes a powerful utility called `ctester` for testing and validating LLM agent capabilities. This tool helps ensure your LLM provider configurations work correctly with different agent types, allowing you to optimize model selection for each specific agent role.
...
If you already have a running penNULL container and want to test the current configuration:
...
penNULL uses vector embeddings for semantic search, knowledge storage, and memory management. The system supports multiple embedding providers that can be configured according to your needs and preferences.

### Supported Embedding Providers

penNULL supports the following embedding providers:
...
EXTERNAL_SSL_CA_PATH=           # Path to custom CA certificate file (PEM format) inside the container
                                # Must point to /opt/pennull/ssl/ directory (e.g., /opt/pennull/ssl/ca-bundle.pem)
...
```bash
# Default location (if PENNULL_SSL_DIR is not set)
cp ca-bundle.pem ./pennull-ssl/

# Or custom location (if using PENNULL_SSL_DIR in docker-compose.yml)
cp ca-bundle.pem /path/to/your/ssl/dir/
```

**Step 3:** Set the path in `.env` file (path must be inside the container):
```bash
# The volume pennull-ssl is mounted to /opt/pennull/ssl inside the container
EXTERNAL_SSL_CA_PATH=/opt/pennull/ssl/ca-bundle.pem
EXTERNAL_SSL_INSECURE=false
```

**Step 4:** Restart penNULL:
```bash
docker compose restart pennull
```

**Notes:**
- The `pennull-ssl` volume is mounted to `/opt/pennull/ssl` inside the container
- You can change host directory using `PENNULL_SSL_DIR` variable in docker-compose.yml
...
penNULL includes a specialized `etester` utility for testing, managing, and debugging embedding functionality. This tool is essential for diagnosing and resolving issues related to vector embeddings and knowledge storage.
...
If you're running penNULL in Docker, you can use etester from within the container:

```bash
# Test embedding provider
docker exec -it pennull /opt/pennull/bin/etester test

# Show detailed database information
docker exec -it pennull /opt/pennull/bin/etester info -verbose
```
...
docker exec -it pennull /opt/pennull/bin/etester search -query "Security vulnerability" -doc_type guide -threshold 0.8

# Filter by flow ID
docker exec -it pennull /opt/pennull/bin/etester search -query "Code examples" -doc_type code -flow_id 42

# All available search options
docker exec -it pennull /opt/pennull/bin/etester search -help
```
...
The `describe` function provides detailed information about tasks and subtasks within a flow. This is particularly useful for diagnosing issues when penNULL encounters problems or gets stuck.
...
When penNULL gets stuck in a flow:
...
If you have penNULL running in Docker, you can use ftester from within the container:

```bash
# Run ftester inside the running penNULL container
docker exec -it pennull /opt/pennull/bin/ftester [arguments]

# Examples:
docker exec -it pennull /opt/pennull/bin/ftester -flow 123 describe
docker exec -it pennull /opt/pennull/bin/ftester -flow 123 terminal -command "ps aux" -message "List processes"
```
...
docker buildx build \
  ...
  --build-arg PACKAGE_VER=$PACKAGE_VER \
  --build-arg PACKAGE_REV=$PACKAGE_REV \
  -t myregistry/pennull:$PACKAGE_VER \
  --push .
```
...
penNULL is built on top of the open-source work of many remarkable people and projects:

- **[PentAGI](https://github.com/vxcontrol/pentagi)** by [vxcontrol](https://github.com/vxcontrol) — the upstream project that penNULL is forked from. All core architecture, Go backend, React frontend, and agent system originate from PentAGI. penNULL rebrands and extends this work under the same MIT License terms.
- [Emerging Architectures for LLM Applications](https://lilianweng.github.io/posts/2023-06-23-agent) — foundational research on LLM agent architectures
- [A Survey of Autonomous LLM Agents](https://arxiv.org/abs/2403.08299) — academic survey underpinning the multi-agent design
- [Codel](https://github.com/semanser/codel) by Andriy Semenets — initial architectural inspiration for agent-based automation
- [gqlgen](https://github.com/99designs/gqlgen), [gin](https://github.com/gin-gonic/gin), [GORM](https://gorm.io/), [pgvector](https://github.com/pgvector/pgvector), [Apollo Client](https://www.apollographql.com/docs/react/), [Radix UI](https://www.radix-ui.com/) — core open-source libraries powering the stack

## License

**penNULL** is licensed under the [MIT License](LICENSE).

Copyright (c) 2026 NullAI Lab — https://github.com/NullAILab

This project is a fork of [PentAGI](https://github.com/vxcontrol/pentagi) (MIT License, Copyright © 2025 vxcontrol). All original code is used in compliance with the MIT License terms.

### Third-Party Dependencies

All third-party dependencies use MIT-compatible licenses. See [licenses/](licenses/) directory for detailed license reports generated by `go-licenses` and `license-checker`.

---

<div align="center">

Built with ❤️ by [NullAI Lab](https://github.com/NullAILab) · Part of the **67 Cybersecurity Projects** portfolio

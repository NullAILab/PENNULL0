#!/bin/bash
#
# Generate license reports for penNULL dependencies
#
...
if command -v go-licenses &> /dev/null; then
    ...
    GOROOT=$(go env GOROOT) GOTOOLCHAIN=auto go-licenses csv ./cmd/pennull > "../$LICENSES_DIR/backend-licenses.csv" 2>/dev/null || {

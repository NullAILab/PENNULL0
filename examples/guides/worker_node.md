# penNULL Worker Node Setup

This guide configures a distributed penNULL deployment where worker node operations are isolated on a separate server for enhanced security. The worker node runs both host Docker and Docker-in-Docker (dind) to provide secure container execution environments.
...
graph TB
    ...
    subgraph "Worker Node"
        ...
        subgraph "Worker Containers"
            WC1[pennull-terminal-1]
            WC2[pennull-terminal-N]
        end
    end
...
**Connection Modes:**
- **Standard**: penNULL → Host Docker (creates workers) → Workers use dind via socket mapping
- **Direct**: penNULL → dind (creates workers directly, socket mapping disabled)
...
**Metrics Integration:** The metrics ports (9323, 9324) can be configured in penNULL's `observability/otel/config.yml` under the `docker-engine-collector` job name for monitoring integration.

### OOB Attack Port Range

Each worker container (`pennull-terminal-N`) dynamically allocates **2 ports** from the range `28000-30000` on all network interfaces to facilitate Out-of-Band (OOB) attack techniques during penetration testing.

**Firewall Requirements:**
- **Inbound**: Allow access to ports 2376, 3376, 9323, 9324 on `${PRIVATE_IP}` from the main penNULL node
...
Copy the client certificates from the worker node to the main penNULL node for secure Docker API access. The certificates need to be transferred to specific directories that the penNULL installer will recognize.
...
scp docker-host-ssl.tar.gz root@<MAIN_NODE_IP>:/opt/pennull/
...
# Transfer to main node (replace <MAIN_NODE_IP> with actual IP)
scp docker-dind-ssl.tar.gz root@<MAIN_NODE_IP>:/opt/pennull/
...
ls -la /opt/pennull/docker-host-ssl/
ls -la /opt/pennull/docker-dind-ssl/
...
These certificate directories will be used by the penNULL installer to configure secure connections to the worker node Docker services.

## Install penNULL on Main Node

After completing the worker node setup and transferring certificates, install penNULL on the main node using the official installer.

### Download and Run Installer

Execute the following commands on the main node to download and run the penNULL installer:

```bash
# Create installation directory and navigate to it
mkdir -p /opt/pennull && cd /opt/pennull
...
After the installer completes and penNULL is running, manually configure the Docker environment through the web interface:

1. **Access penNULL Installer** via `./installer`
...
- **Docker Network**: `pennull-network` (custom network name)
...
- **Pentesting Image**: `nullailab/kali-linux` (or leave empty)
- **Docker Host**: `tcp://${PRIVATE_IP}:2376` (TLS connection to host Docker)
- **TLS Verification**: `1` (enable TLS verification)
- **TLS Certificates**: `/opt/pennull/docker-host-ssl` (path to client certificates)

**For Direct Mode (dind only):**
- Use the same configuration but change:
- **Docker Host**: `tcp://${PRIVATE_IP}:3376` (TLS connection to dind)
- **TLS Certificates**: `/opt/pennull/docker-dind-ssl` (path to dind client certificates)
- **Docker Socket**: Leave empty (no socket mapping needed)

The certificate directories `/opt/pennull/docker-host-ssl/` and `/opt/pennull/docker-dind-ssl/` will be automatically mounted into the penNULL container for secure TLS authentication.

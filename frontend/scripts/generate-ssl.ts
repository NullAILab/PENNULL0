...
export const generateCertificates = (): void => {
    ...
    -subj "/C=US/ST=NY/L=NY/O=penNULL/OU=Project/CN=penNULL CA" \
    ...
    -subj "/C=US/ST=NY/L=NY/O=penNULL/OU=Project/CN=localhost" \
    -out ${SSL_PATHS.serverCsr}`,
    );

    // Create temporary configuration file
    const extFile = join(SSL_PATHS.sslDir, 'extfile.tmp');
    const extFileContent = ['subjectAltName=DNS:pennull.local', 'keyUsage=critical,digitalSignature,keyAgreement'].join(
        '\n',
    );
